...
// PB Optimization groups for pre- and post-FDO inlining
static cl::opt<bool>
FDIpre("FDI-Opre", cl::desc("Use pre-instrumentation FDI opt group"),
       cl::init(false));

static cl::opt<bool>
FDIpost("FDI-Opost", cl::desc("Use post-FDI opt group"),
       cl::init(false));
...
int main(int argc, char **argv) {
  ...
  // PB if (OptLevelO1 || OptLevelO2 || OptLevelO3) {
  if (OptLevelO1 || OptLevelO2 || OptLevelO3 || FDIpre || FDIpost) {
  ...
  errs() << "OPT: number of passes in pass list: " << PassList.size() << "\n";

  if(FDIpre || FDIpost)
  {
    createFDIFunctionPasses(&(*FPasses));
    if(FDIpre) createFDIPreOptPasses(&Passes);
    if(FDIpost) createFDIPostOptPasses(&Passes);
  }
  else
  {
