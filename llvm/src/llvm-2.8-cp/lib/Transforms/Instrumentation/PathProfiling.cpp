...
namespace {
  ...
  private:
    ...
    // oldVal + addition.
    void insertNumberIncrement(BLInstrumentationNode* node, Value* addition, bool atBeginning);
...
namespace llvm {
	...
	template<bool xcompile> class TypeBuilder<PathProfilingFunctionTable, xcompile> {
  public:
    static const StructType *get(LLVMContext& C) {
      return( StructType::get(C,
	    TypeBuilder<types::i<32>, xcompile>::get(C), // type
	    TypeBuilder<types::i<32>, xcompile>::get(C), // array size
	    TypeBuilder<types::i<8>*, xcompile>::get(C), // array/ hash pointer
	    NULL));
    }
	};
...
BLInstrumentationEdge::BLInstrumentationEdge(BLInstrumentationNode* source,
    BLInstrumentationNode* target) : BallLarusEdge(source, target, 0),
...
void BLInstrumentationDag::calculateSpanningTree() {
  ...
  for(BLEdgeIterator edge = _edges.begin(), end = _edges.end();
    ...
    if(!instEdge->isInSpanningTree() && (*edge)->getType() != BallLarusEdge::SPLITEDGE)
...
void BLInstrumentationNode::setStartingPathNumber(Value* pathNumber) {
	DEBUG(dbgs() << "  SPN-" << getName() << " <-- " << (pathNumber ? pathNumber->getNameStr() : "unused") << "\n");
...
void BLInstrumentationNode::setEndingPathNumber(Value* pathNumber) {
	DEBUG(dbgs() << "  EPN-" << getName() << " <-- " << (pathNumber ? pathNumber->getNameStr() : "unused") << "\n");
...
static RegisterPass<PathProfiler>
X("insert-path-profiling",
...
void PathProfiler::pushValueIntoNode(BLInstrumentationNode* source,
  ...
  if(target->getNumberPredEdges() <= 1) {
    assert(target->getStartingPathNumber() == NULL && "Target already has path number");
    ...
    DEBUG(dbgs() << "  Passing path number"
			<< (source->getEndingPathNumber() ? "" : " (null)") << " value through.\n");
  } else {
    if(target->getPathPHI() == NULL) {
    	DEBUG(dbgs() << "  Initializing PHI node for block '" << target->getName() << "'\n");
      preparePHI(target);
    }
    pushValueIntoPHI(target, source);
    DEBUG(dbgs() << "  Passing number value into PHI for block '" << target->getName() << "'\n");
  }
}
...
void PathProfiler::insertNumberIncrement(BLInstrumentationNode* node,
  ...
  Value* newpn = BinaryOperator::Create(Instruction::Add,
		node->getStartingPathNumber(), addition, "pathNumber", insertPoint);
...
void PathProfiler::insertCounterIncrement(Value* incValue,
		BasicBlock::iterator insertPoint,
		BLInstrumentationDag* dag, bool increment) {
    ...
    GetElementPtrInst* pcPointer =
      GetElementPtrInst::Create(dag->getCounterArray(),
				gepIndices.begin(), gepIndices.end(), "counterInc", insertPoint);
    ...
    LoadInst* oldPc = new LoadInst(pcPointer, "oldPC", insertPoint);
		...
		ICmpInst* isMax = new ICmpInst(insertPoint, CmpInst::ICMP_ULT,
			oldPc, createIncrementConstant(0xffffffff, 32), "isMax");

		// Select increment for the path counter based on overflow
		SelectInst* inc = SelectInst::Create(isMax, createIncrementConstant(increment?1:-1,32),
...
void PathProfiler::insertInstrumentationStartingAt(BLInstrumentationEdge* edge,
    ...
    (BLInstrumentationEdge*) edge->getPhonyExit();
		...
		BasicBlock::iterator insertPoint = atBeginning ? instrumentNode->getBlock()->getFirstNonPHI() : instrumentNode->getBlock()->getTerminator();

		// add information from the bottom edge, if it exists
		if( bottom->getIncrement() ) {
			Value* newpn = BinaryOperator::Create(Instruction::Add,
				instrumentNode->getStartingPathNumber(), createIncrementConstant(bottom), "pathNumber", insertPoint);
		...
		if( top->isCounterIncrement() ) {
			insertCounterIncrement(instrumentNode->getEndingPathNumber(), instrumentNode->getBlock()->getTerminator(),dag);
	...
	// Insert instrumentation if this is a normal edge
		...
		if( edge->isInitialization() ) { // initialize path number
			...
			Value* newpn = BinaryOperator::Create(Instruction::Add,
				instrumentNode->getStartingPathNumber(), createIncrementConstant(edge), "pathNumber", insertPoint);
  ...
  insertInstrumentationStartingAt(exitRootEdge, &dag);
    ...
    while( ((Instruction&)(*insertPoint)).getOpcode() != Instruction::Call )
		...
		if( node->getStartingPathNumber() ) {
			if ( ((BLInstrumentationEdge*)(*edge))->getIncrement() )
				newpn = BinaryOperator::Create(Instruction::Add, node->getStartingPathNumber(),
					createIncrementConstant(((BLInstrumentationEdge*)(*edge))->getIncrement(),32), "pathNumber", insertPoint);
			else
				newpn = node->getStartingPathNumber();
		} else {
			newpn = (Value*)createIncrementConstant(((BLInstrumentationEdge*)(*edge))->getIncrement(), 32);
		}

		insertCounterIncrement(newpn, insertPoint, &dag);
		insertCounterIncrement(newpn, node->getBlock()->getTerminator(), &dag, false);
	}
}

// Entry point of the module
void PathProfiler::runOnFunction(std::vector<Constant*> &ftInit, Function &F, Module &M) {
  // Build DAG from CFG
  ...
  if (DotPathDag)
	...
	if( dag.getNumberOfPaths() <= HASH_THRESHHOLD ) {
		...
		dag.setCounterArray(new GlobalVariable(M, t, false,
			GlobalValue::InternalLinkage, Constant::getNullValue(t), ""));
	}
  ...
  ConstantStruct* functionEntry = (ConstantStruct*)ConstantStruct::get(at, entryArray);
...
bool PathProfiler::runOnModule(Module &M) {
  ...
  llvmIncrementHashFunction = M.getOrInsertFunction("llvm_increment_path_count",
      Type::getVoidTy(*Context), // return type
      Type::getInt32Ty(*Context), // function number
      Type::getInt32Ty(*Context), // path number
      NULL );

	llvmDecrementHashFunction = M.getOrInsertFunction("llvm_decrement_path_count",
      Type::getVoidTy(*Context), // return type
      Type::getInt32Ty(*Context), // function number
      Type::getInt32Ty(*Context), // path number
      NULL );
  ...
  GlobalVariable* functionTable = new GlobalVariable(M, ftArrayType,
      false, GlobalValue::InternalLinkage, ftInitConstant,
      "functionPathTable");

  InsertProfilingInitCall(Main, "llvm_start_path_profiling", functionTable,
		PointerType::getUnqual(ftArrayType->getTypeAtIndex((unsigned)0)));
