...
namespace {
  class PathProfileVerifierPass : public ModulePass {
  private:
    bool runOnModule(Module &M);

  public:
    static char ID; // Pass identification, replacement for typeid
    PathProfileVerifierPass() : ModulePass(ID) {}
...
char PathProfileVerifierPass::ID = 0;
static RegisterPass<PathProfileVerifierPass> X("path-profile-verifier",
    "Compare the path profile derived edge profile against the edge profile.");

ModulePass *llvm::createPathProfileVerifierPass() {
  return new PathProfileVerifierPass();
}

// The verifier requires the path profile and edge profile.
void PathProfileVerifierPass::getAnalysisUsage(AnalysisUsage& AU) const {
...
bool PathProfileVerifierPass::runOnModule (Module &M) {
  ...
  std::vector<unsigned> edgeArray(i);
    ...
    if (F->isDeclaration()) continue;
		...
		DEBUG(dbgs() << "function '" << F->getName() << "' ran "
			...
			<< "/" << pathProfileInfo.getPotentialPathCount() << " potential paths\n");

		for( PathIterator nextPath = pathProfileInfo.pathBegin(),
				endPath = pathProfileInfo.pathEnd(); nextPath != endPath; nextPath++ ) {
			Path* currentPath = nextPath->second;

			PathEdgeVector* pev = currentPath->getPathEdges();
		DEBUG(dbgs () << "path #" << currentPath->getNumber() << ": " << currentPath->getCount() << "\n");
			...
			for( PathEdgeIterator nextEdge = pev->begin(), endEdge = pev->end();
					nextEdge != endEdge; nextEdge++ ) {
				...
				// Ensure all the referenced edges exist TODO: make this a separate function
				if( !arrayMap.count(source) ) {
					errs() << "  error [" << F->getNameStr() << "()]: source '"
						<< source->getNameStr() << "' does not exist in the array map.\n";
				} else if( !arrayMap[source].count(target) ) {
					errs() << "  error [" << F->getNameStr() << "()]: target '"
						<< target->getNameStr() << "' does not exist in the array map.\n";
