...
// Initializes a new Node for the given BasicBlock
BallLarusNode::BallLarusNode(BasicBlock* BB) :
...
// Initializes an BallLarusEdge with a source and target.
BallLarusEdge::BallLarusEdge(BallLarusNode* source, BallLarusNode* target,
	unsigned duplicateNumber) : _source(source), _target(target),
	_weight(0), _edgeType(NORMAL), _realEdge(NULL),
...
// Initializes a BallLarusDag from the CFG of a given function.
BallLarusDag::BallLarusDag(Function &F) :
...
// get the first edge of the path
// PB: taken directly from llvm-cprof.cpp, where it is called on a root.
BallLarusEdge* BallLarusDag::getFirstBLEdge(unsigned int pathNumber) {
  BallLarusEdge* best = 0;

  if(!_root){
    errs() << "getFirstBLEdge: DAG has no root!\n";
    return(0);
  }

  for( BLEdgeIterator next = _root->succBegin(),
         end = _root->succEnd(); next != end; next++ ) {
...
void BallLarusDag::buildEdge(BLBlockNodeMap& inDag, std::stack<BallLarusNode*>&
    dfsStack, BallLarusNode* currentNode, BasicBlock* succBB,
...
BallLarusEdge* BallLarusDag::createEdge(BallLarusNode* source,
    BallLarusNode* target, unsigned duplicateCount) {
