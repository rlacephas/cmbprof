...
//PB: needed for originating callsite vector
...
public:
  ...
  // parallel array of the original calls that InlinedCalls originate from
  SmallVector<CallSite, 8> InlinedCallOrigins;
  
  void reset() {
    StaticAllocas.clear();
    InlinedCalls.clear();
    InlinedCallOrigins.clear();
  }
};
